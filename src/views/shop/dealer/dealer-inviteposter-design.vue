    ...
    <div class="footbtn">
      <foot-button>
        <Button
          ...
          perm="dealer.dealerinvite.operate"
          :loading="isLoading"
        >保存</Button>
      </foot-button>
    </div>
...
import PaperProperty from "./components/authcert-modules-for-design/InvitePosterProperty";
...
export default {
  ...
  methods: {
    loadTemplate() {
      // 点击编辑跳转进来
      axios
        .get("/shop/admin/dealer/dealerinvite/inviteposter/info", {
    ...
    addModule(type) {
      if (["ModuleNickName", "ModuleHead", "ModuleQrcode", "ModuleLevel"].indexOf(type) > -1) {
      ...
      this.info.modules.push(ModuleHelper.newModule(type));
    },
    editModule(module) {
    ...
    save() {
      ...
      if (!this.myValidatorResult) {
        ...
        this.$httpPost(
          "/shop/admin/dealer/dealerinvite/inviteposter/save?id=" + this.info.id,
          { info: objToSave },
          res => {
            if (res.code == 200) {
