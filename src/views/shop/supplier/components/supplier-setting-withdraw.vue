<template>
<div class="supplier-setting-withdraw">
  <div class="content">
    <Form ref="form" :label-width="140" :model="formData" :rules="rules">
      <div v-show="false" class="title-block">收入提现设置
      ...
      <div v-show="!allClosed">
        ...
        <FormItem style="margin-bottom:0!important" label="单笔提现额度：" class="ivu-form-item-required">
          <FormItem class="form-item-inner" prop="min_money">
            <InputNumber :min="1" :max="9999999" :precision="0" v-model="formData.min_money" placeholder="请输入额度下限"/>
            </FormItem>
          <span style="color:#dcdee2;margin:0 5px;">—</span>
          <FormItem class="form-item-inner" prop="max_money">
            <InputNumber :min="1" :max="9999999" :precision="0" v-model="formData.max_money" style="margin-right: 5px;" placeholder="请输入额度上限"/>元
          </FormItem>
        </FormItem>
  ...
  <div class="footer">
    <Button type="primary" shape="circle" @click="saveSetting" perm="supplier.config.operate">保存</Button>
  </div>
</div>
</template>
<script>
...
export default {
  ...
  data() {
    ...
    const maxValidator = (rule,value,callback) => {
      ...
      this.$refs.form.validateField("money")
      if(!value || this.formData.min_money && value < this.formData.min_money) {
        return callback(" ")
      } else {
        if(!lock){
          lock = true
  ...
  methods: {
    ...
    getSetting() {
      ...
      axios.get("/shop/admin/supplier/withdrawconfig/info").then(res => {
        if(res.code == 200) {
    ...
    saveSetting() {
      let data = {}
      data.offline_type = this.formData.offline_type;
      ...
      this.$refs.form.validate(valid => {
        if(valid) {
          this.$store.state.COMMON_loading = true
          axios.get("/shop/admin/supplier/withdrawconfig/edit", {
            params: data
          }).then(res => {
            if(res.code == 200) {
