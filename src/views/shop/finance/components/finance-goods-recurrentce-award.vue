      ...
      columnsData: [
        ...
        },
        {
          ...
          render: (h, p) => {
            return h("div", { on:{
              click: () => {
                this.$router.push({name:"cloudstockOrderDetailBuy",query:{order_id: p.row.order_id}})
              }
            },
              style:{
                cursor: 'pointer'
              ...
              on: {
                click: () => {
                  if(p.row.status == 0) {
                    this.$Modal.confirm({
                      title: "提示",
                      content: `确定要帮【${p.row.member_nickname}】兑换该笔订货返现奖金【￥${p.row.reward_money}】吗？`,
                      onOk:() => {
  ...
  methods: {
    ...
    exprotData(data) {
      commonFunction.exportTable(
        data,
        this.http,
        "/shop/admin/dealer/order/reward/export"
      );
    },
    getSearchData() {
      return {
    ...
    getAwardList() {
      ...
      this.http.post("/shop/admin/dealer/order/reward/list", this.getSearchData()).then(res => {
