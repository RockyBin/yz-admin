<template>
  <div>
    <Modal
      ...
      :width="468"
      :title="title"
      class="finance-sales-award-verify-modal"
      :class="{
        'finance-sales-award-verify-modal-cancel': !(
          verifyInfo.status == 1 && !verifyInfo.pay_member_id
        )
      }"
      :loading="loading"
      @on-ok="confirm"
    >
      <Form ref="form" :labelWidth="86" :model="formFields" :rules="rules">
        ...
        <div
          v-if="
            [2, 3].includes(verifyInfo.status) ||
              (verifyInfo.status == 1 && verifyInfo.pay_member_id) ||
              (verifyInfo.status == 1 && formFields.status)
          "
          class="custom-form-item"
        >
          <span class="label">推荐奖金：</span>
          <span class="content"><span>¥{{ verifyInfo.reward_money }}</span>（{{
              !verifyInfo.pay_member_id ? "公司支付" : "上级支付"
            }}）</span>
        </div>
        <div
          v-if="
        ...
        >
          <div class="input-wrapper">
            ...
            />
            <span class="suffix"
              >{{ formFields.reason ? formFields.reason.length : 0 }}/20</span
            >
          </div>
        </FormItem>
      </Form>
        ...
        <div class="related-member">
          <div v-if="verifyInfo.pay_member_id">
            ...
            <div style="margin-bottom: 16px;">
              <img
                :src="
                  formatURL(verifyInfo.pay_member_headurl) ||
          ...
          <div>
            ...
            <div>
              <img
                :src="
                  formatURL(verifyInfo.member_headurl) ||
                    require('../../images/head-portrait.png')
                "
                alt=""
              />
    ...
    progressList() {
      ...
      if (this.verifyInfo.status == 1) {
        list.push({
          ...
          title: "上级审核中",
          content: `审核人：${
            this.verifyInfo.pay_member_id
              ? this.verifyInfo.pay_member_name
              : "公司"
          }`
        });
        list.push({
  ...
  methods: {
    ...
    confirm() {
      if (this.verifyInfo.status == 1 && !this.verifyInfo.pay_member_id) {
        this.$httpPost(
          "/shop/admin/dealer/reward/verify",
          {
