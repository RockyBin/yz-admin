<template>
  <div class="product-detail-table-multi-sale" :style="'width:' + tableWidth + 'px'">
    <table
      class="table-wrapper"
      cellspacing="0"
      cellpadding="0"
      ...
      <thead>
        <tr>
          ...
          <th class="title-th" v-show="commissionType !== -1">
            <span>
              <label class="product-detail-specifications-required"></label>订单返佣
            </span>
            <div>
              <a
                href="javacript:;"
      ...
      <tbody>
        ...
        <tr v-for="(item,index) in value.skuArray" :key="index">
          ...
          <td
            ...
            :class="{'product-detail-table-multi-sale-error' : commissionType === 1  && setArray['multi_sale_error_' + item.sku_code]}"
            v-show="commissionType !== -1"
            @click="onViewAndSetCommissionClick(item)"
          >{{commissionType === 0 ? '点击查看' : item.agent_sale_reward_info && Object.values(item.agent_sale_reward_info.rule.commission).filter(n => n !== null).length === 3 ? '修改设置' : '去设置'}}</td>
        </tr>
...
<script>
import modalSale from "./product-detail-modal-sale";
export default {
  props: {
    value: {
      type: Object,
  ...
  methods: {
      ...
      if (this.commissionType === 1) {
        status =
          this.value.skuArray.filter(item => {
            ...
            if (item.agent_sale_reward_info) {
              let values = item.agent_sale_reward_info.rule;
              status1 =
                Object.keys(values.commission).length !== 3 ||
                Object.values(values.commission).includes(null) || values.low_commission === null;
            } else {
              status1 = true;
            }
            ...
            if (flag) {
              if (status1) {
                this.$set(this.setArray, "multi_sale_error_" + item.sku_code, true);
              }
            }
            // 为true则是不合格
            return status1;
    ...
    onViewAndSetCommissionClick(item) {
      ...
      this.selectSku = {
        agent_sale_reward_info: item.agent_sale_reward_info
          ? JSON.parse(JSON.stringify(item.agent_sale_reward_info))
          : null
      };

      if (this.agentConfig.agent_sale_reward_config && this.agentConfig.agent_sale_reward_config.commision_type) {
        this.priceDiff = typeof item.price === "number" ? item.price : null;
      } else {
        this.priceDiff = typeof item.supply_price === "number" && typeof item.price === "number" ? (item.price - item.supply_price > 0 ? item.price - item.supply_price : 0) : null;
      }
    ...
    onCommissionModalComfirm() {
      if (this.memberSkuString === null) {
        ...
        this.value.skuArray.forEach(item => {
          delete this.setArray["multi_sale_error_" + item.sku_code];
          this.$set(
            item,
            "agent_sale_reward_info",
            JSON.parse(
              JSON.stringify(this.selectSku.agent_sale_reward_info)
            )
          );
        });
      } else {
        this.$set(
          ...
          "agent_sale_reward_info",
          this.selectSku.agent_sale_reward_info
        );
        delete this.setArray[
          "multi_sale_error_" + this.selectSkuDataItem.sku_code
        ];
      }
    }
  }
...
.product-detail-table-multi-sale {
  overflow-x: auto;
  ...
  table.table-wrapper {
    ...
    tr td.product-detail-table-multi-sale-error {
      color: #ed4014;
    }
    .serial-num-input {
      width: 122px;
