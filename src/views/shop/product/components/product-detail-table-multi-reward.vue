<template>
  <div class="product-detail-table-multi-reward" :style="'width:' + tableWidth + 'px'">
    <table
      class="table-wrapper"
      cellspacing="0"
      cellpadding="0"
      ...
      <thead>
        <tr>
          ...
          <th class="title-th" v-show="commissionType !== -1">
            <span>
              <label class="product-detail-specifications-required"></label>订单分红
            </span>
            <div>
              <a
                href="javacript:;"
      ...
      <tbody>
        ...
        <tr v-for="(item,index) in value.skuArray" :key="index">
          ...
          <td
            ...
            :class="{'product-detail-table-multi-reward-error' : commissionType === 1  && setArray['multi_reward_error_' + item.sku_code]}"
            v-show="commissionType !== -1"
            @click="onViewAndSetCommissionClick(item)"
          >{{commissionType === 0 ? '点击查看' : item.agent_order_commission_info && Object.values(item.agent_order_commission_info.rule.commission).filter(n => n !== null).length === 3 ? '修改设置' : '去设置'}}</td>
        </tr>
...
<script>
import modalReward from "./product-detail-modal-reward";
export default {
  props: {
    value: {
      type: Object,
  ...
  methods: {
      ...
      if (this.commissionType === 1) {
        status =
          this.value.skuArray.filter(item => {
            ...
            if (item.agent_order_commission_info) {
              let values = Object.values(
                item.agent_order_commission_info.rule.commission
              );
              status1 =
            ...
            if (flag) {
              if (status1) {
                this.$set(this.setArray, "multi_reward_error_" + item.sku_code, true);
              }
            }
            // 为true则是不合格
            return status1;
    ...
    onBatchSettingCommisionClick() {
      ...
      this.selectSku = {
        agent_order_commission_info: null
      };
    ...
    onViewAndSetCommissionClick(item) {
      ...
      this.selectSku = {
        agent_order_commission_info: item.agent_order_commission_info
          ? JSON.parse(JSON.stringify(item.agent_order_commission_info))
          : null
      };

      if (this.agentConfig.agent_base_config && this.agentConfig.agent_base_config.commision_type) {
    ...
    onCommissionModalComfirm() {
      if (this.memberSkuString === null) {
        ...
        this.value.skuArray.forEach(item => {
          delete this.setArray["multi_reward_error_" + item.sku_code];
          this.$set(
            item,
            "agent_order_commission_info",
            JSON.parse(
              JSON.stringify(this.selectSku.agent_order_commission_info)
            )
          );
        });
      } else {
        this.$set(
          ...
          "agent_order_commission_info",
          this.selectSku.agent_order_commission_info
        );
        delete this.setArray[
          "multi_reward_error_" + this.selectSkuDataItem.sku_code
        ];
      }
    }
  }
...
.product-detail-table-multi-reward {
  overflow-x: auto;
  ...
  table.table-wrapper {
    ...
    tr td.product-detail-table-multi-reward-error {
      color: #ed4014;
    }
    .serial-num-input {
      width: 122px;
